# from hdpitkinter.high_dpi_tkinter import HdpiTk # for high dpi awareness
from tkinter import *
...
class EVcars():
    def __init__(self):
        ...
        self.window = Tk() # used HdpiTk() instead of Tk() to aware high dpi
        # Set high DPI awareness
        self.window.tk.call('tk', 'scaling', 1.25)  # Adjust the scaling factor as
    ...
    def start_page(self):
        ...
        start_pg_ico = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/start_pg.png").resize((123, 40)))
        ...
        quit_ico = PhotoImage(file="./_internal/frame_logo/quit.png")
        ...
        self.warn_box = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/warn_box.png").resize((170,120)))
        self.back_sq_img = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/back_sq.png").resize((60,37)))
        self.next_img = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/next_sq.png").resize((60, 37)))
        self.cancel_img = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/cancel_sq.png").resize((60,37)))
        self.confirm_img = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/confirm_sq.png").resize((60,37)))
    ...
    def load_file(self):
        ...
        if self.file:
            ...
            resized_noti_ico = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/start_pg.png").resize((149, 50)))
            noti_box = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/noti_box.png").resize((280, 80)))
    ...
    def home(self):
        ...
        img = ImageTk.PhotoImage(Image.open("./_internal/frame_logo/start_img.png").resize((800, 600)))
        ...
        Label(show_frame, text="Welcome!", font=("Helvectica", 35), borderwidth=0, fg="blue", bg="white").grid(row=0, column=0, pady=200, sticky=N)
        Label(show_frame, text="Click to start...", font=("", 20), borderwidth=0, fg='blue', bg='darkblue').grid(row=0, column=0, pady=70, sticky=S)
    ...
    def show_cars_info_ico(self):
        ...
        self.window.title("Ninjas EV Management System") # 14/6/24 change ico and title back to normal cuz I change in models frame
        ...
        if self.datas:
            ...
            # self.menuBar.entryconfig(" Info ", state="active")            
            self.menuBar.entryconfig(" Home ", state=DISABLED)
    ...
    def update(self):
        ...
        ori_model_img = [img for img in os.listdir("./_internal/models/"+self.selected_lst[0].lower()) if img == self.selected_lst[1]+".png"]
        if model_img not in [model for model in os.listdir("./_internal/models")]:
            # os.mkdir("./Project/"+self.modified_lst[0].lower())
            img = Image.open(model_img)
            img.save("./_internal/models/"+self.selected_lst[0].lower()+"/"+img)
        ...
        # self.show_txt.grid(row=0, column=0, sticky="we")#, ipady=5, ipadx=350)
            ...
            for i in diff: # eg. i = (2, '57') like (column_index, its value)
                ...
                self.edit_info.insert(END, f" {self.selected_lst[i[0]]} ", "original val") # I want to add color text, but it doesn't
    ...
    def del_directly(self):
        ...
        if response:
            ...
            if complete:
                self.tree_search.delete(*self.tree_search.get_children())
                # self.tree_search["columns"] = ()
                self.search_Info() # to show the existing data after deleting        else:
        else:
            pass
