...
class EVcars():
    ...
    def home(self):
        ...
        # evcar bg ="#D6FCEE", moving car bg =#6464bb
        a_img = Image.open("./Project/gif/evcar.gif")
        ...
        Label(self.show_frame, text="Welcome", font=("Helvectica", 35), borderwidth=0, bg="#D9F8FC").grid(row=0, column=0, sticky=N)

        log_in_frame = Frame(self.show_frame, bg="grey")
        log_in_frame.grid(row=0, column=0, sticky="n",pady=100)
        Label(log_in_frame, text="Log In", fg="white", bg="grey", font=("Helvectica", 15)).grid(row=0, column=0)
        
        Label(log_in_frame, text="Username", fg="white", bg="grey")
        opt_lst = ["User", "Admin"]
        self.style.theme_use("vista")
        self.combobox = ttk.Combobox(log_in_frame, values=opt_lst, state="readonly", width=15, height=10)
        ...
        self.pass_ent = Entry(log_in_frame, show="*", width=17)
        ...
        self.quit_ico = PhotoImage(file="./Project/frame_logo/door.png")
    ...
    def show_about(self, labl, labl2, prev_msg, msgs):
        ...
        try:
            ...
            # I used try , except statement to bypass TclError
            try:
    ...
    def load_file(self):
        ...
        initialdir="C:/Users/Desktop/", #../MainFrame/Desk...
        ...
        if self.file:
            ...
            # self.noti_box = ImageTk.PhotoImage(Image.open("./Project/frame_logo/checked.png")) # moved cuz of not isAdmin
    ...
    def start_page(self):
        ...
        self.start_frame = Frame(self.window, bg="#838B8B") #lavenderblush4
        self.start_frame.grid(row=0, column=0, sticky="nsew")

        if not self.isAdmin:  ##  <=====
            self.file = "./evcars_subset.csv"
            self.datas = self.data_upload(self.file)
            # data1= self.data_upload("") # for more csv file
        ...
        # self.style = ttk.Style()
        self.style.theme_use('default')  # try with different theme ('clam', etc..)
        ...
        self.load_a_ico = Image.open("./Project/frame_logo/data1.gif") # to use in hover event
        self.load_ico = ImageTk.PhotoImage(Image.open("./Project/frame_logo/data1.gif"))

        self.noti_box = ImageTk.PhotoImage(Image.open("./Project/frame_logo/checked.png")) # <==
        ...
        img= Image.open("./Project/gif/convertible-car.gif")#.resize((100, 100))
        ...
        logout_a_ico = Image.open("./Project/gif/log-out.gif")
        logout_ico = PhotoImage(file="./Project/frame_logo/door.png")
    ...
    def explore_Info(self):
        if self.datas:
            ...
            self.entries = []
            # value = StringVar()
            self.up_img_a = Image.open("./Project/gif/upload.gif")
            self.up_img = ImageTk.PhotoImage(Image.open("./Project/gif/upload.gif"))            
            

            for i in range(len(self.fst_half)): # 1 word length = 8 screenunit
                Label(self.frame1, text=f"{self.fst_half[i]} : ",background="lightblue", font=("Consolas", 11, "bold"), wraplength=184).grid(row=i, column=0, padx= 10,  pady=5, sticky=W)
            ...
            if self.isAdmin:
                up_img_btn = Button(self.frame1, image=self.up_img, text=" Upload Image", compound="left", font=("Consolas", 11, "bold"), width=130, height=18, bg="lightblue", command=lambda: self.upload_image(self.img_e))
    ...
    def search_Info(self): ## need to modify , try to search with index
        ...
        self.num_col = ["Top_Speed", "Range(km)", "Efficiency(Wh/km)", "Fastcharge", "Estimated_US_Value", "Towing_capacity_in_kg", "Number_of_seats"]
    ...
    def edit_directly(self):
        ...
        img_path = f"{self.path}/Project/models/{self.selected_lst[0].lower()}/{self.selected_lst[1]}.png"
    ...
    def update(self):
        ...
        try:
            ...
            ori_img_path =f"{self.path}/Project/models/{self.selected_lst[0].lower()}/{ori_model_img[0]}"
            ...
            if edit_model_name != ori_model_img[0] or edit_model_path != ori_img_path:
                Image.open(img_path+"/"+ori_model_img[0]).save("./Project/del_imgs/"+"del_"+ori_model_img[0]) #edit  
    ...
    def show_icon(self,name, event):
        ...
        self.img3 = ImageTk.PhotoImage(Image.open("./Project/brands/"+self.name.lower()+".png").resize((30, 30)))
    ...
    def image_display(self):
        ...
        for i, column in enumerate(self.datas[0][7:]):
            Label(self.txt_frame, text=column, background="skyblue", font=("Helvectica", 10, "bold")).grid(row=i, column=3, sticky=E, padx=20, pady=5)
            Label(self.txt_frame, text=f"- {self.model[0][7:][i]}", justify='right', background="skyblue", font=("Helvectica", 10, "bold")).grid(row=i, column=4, sticky=E, pady=5)
    ...
    def load_images(self, path):
        ...
        for image in images_dir:
            ...
            images.append(ImageTk.PhotoImage(Image.open(path+image).resize((400,250))))
